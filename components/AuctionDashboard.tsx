...
export function AuctionDashboard({ onSelectAuction }: AuctionDashboardProps) {
  ...
  const handleManualRefresh = async () => {
    try {
      ...
      const res = await fetch('http://127.0.0.1:5000/api/auction/get-auction');
      if (res.ok) console.log('✅ Auctions refreshed manually.');
    } catch (err) {
            ...
            >
              <div className="p-4">
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span
                    className={`px-2 py-1 text-xs font-medium rounded-full border ${statusColors[auction.status]}`}
                  >
